...
import { CustomerUserProfileFormType, customerUserProfileFormSchema } from './schema'

export function CustomerUserProfileForm() {
  ...
  const form = useForm<CustomerUserProfileFormType>({
    resolver: zodResolver(customerUserProfileFormSchema),
  ...
  const handleSubmitWithTransition = async (formData: CustomerUserProfileFormType) => {
    startTransition(() => {
      handleSubmit(formData)
    })
  }

  const handleSubmit = async (data: CustomerUserProfileFormType) => {
    ...
    try {
      ...
      if (response.status === 204) {
        ...
        router.push(`${DEFAULT_PAGES.CUSTOMER.profile}?${createQueryString({ profileDetailMode: 'view' })}`, { scroll: false })
      ...
      >
        ...
        {isViewing ? (
          <Button
            ...
            onClick={() => {
              router.push(
                  `${DEFAULT_PAGES.CUSTOMER.profile}?${createQueryString({ profileDetailMode: 'edit' })}`,
                  { scroll: false },
              )
            }}
          >
          ...
          <div className="flex flex-1 items-center justify-end gap-2">
            <Button
              ...
              onClick={() => {
                router.push(
                  `${DEFAULT_PAGES.CUSTOMER.profile}?${createQueryString({ profileDetailMode: 'view' })}`,
                  { scroll: false },
                )
              }}
            >
