...
export function DataTableToolbar<TData>({
  ...
  const [filterValue, setFilterBy] = React.useState(filterBy || 'appointmentRelations.user.name')
  ...
  }, [pathname, router, debouncedValue, filterValue])

  const placeholder = React.useMemo(() => {
    switch (filterValue) {
      case 'appointmentRelations.user.name':
        return 'Nome...'

      case 'appointmentRelations.room.name':
        return 'Sala...'
            ...
            searchableColumns.map((column) => {
              if (column.value === 'search') {
                return (
                  ...
                  >
                    <Select value={filterValue} onValueChange={setFilterBy}>
                      <SelectTrigger className="ml-2 flex h-8 w-10 items-center justify-center gap-2 outline-none">
                        <SearchIcon className="size-3 shrink-0 text-muted-foreground sm:size-4" />
                        <ChevronDown className="size-3 shrink-0 opacity-50 sm:size-4" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectGroup>
                          ...
                          <SelectItem value="appointmentRelations.user.name">Nome</SelectItem>
                          <SelectItem value="appointmentRelations.room.name">Sala</SelectItem>
                        </SelectGroup>
                      </SelectContent>
                    </Select>
                    <Input.Content
