...
import { UpdateAdminAppointmentType, updateAdminAppointmentSchema } from './schema'

interface AppointmentFormProps {
  data: UpdateAdminAppointmentType
  onClose?: () => void
}
...
export function AppointmentTableForm({ data, onClose }: AppointmentFormProps) {
  ...
  const form = useForm<UpdateAdminAppointmentType>({
    resolver: zodResolver(updateAdminAppointmentSchema),
  ...
  const handleSubmitWithTransition = async (formData: UpdateAdminAppointmentType) => {
    startTransition(() => {
      handleSubmit(formData)
    })
  }

  const handleSubmit = async (data: UpdateAdminAppointmentType) => {
              ...
              <FormControl>
                <Input.Root hasError={!!error} readOnly>
                  ...
                  {!isCanceled && !isViewing && (
                    <RoomDialog upApptRoomAdminForm={{ ...form, formId: 'up-appt-room-admin' }} />
                  )}
                </Input.Root>
              </FormControl>
              <FormMessage />
            ...
            <FormItem className="w-full">
              ...
              <FormControl>
                <Input.Root hasError={!!error} readOnly>
                  ...
                  />
                  {!isCanceled && !isViewing && (
                    <AvailableTimesRoomDialog upAvailableTimesAdmForm={{ ...form, formId: 'up-avl-times-adm' }} />
                  )}
                </Input.Root>
              </FormControl>
              <FormMessage />
        ...
        {isViewing && !isCanceled ? (
          <div className="flex w-full items-center justify-end gap-2">
            ...
            <Button
              ...
              onClick={() => {
                router.push(
                  `${pathname}?${createQueryString({ appointmentDetailsMode: 'edit' })}`,
                  { scroll: false },
                )
              }}
            >
